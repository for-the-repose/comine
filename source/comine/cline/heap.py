...
class CHead(CLines):
    def __init__(s):
        CLines.__init__(s, 'heap')

    def __sub_heap_disq(s, infer, argv):
        argv = argv or [ 'do' ]
    ...
    def __sub_heap_lookup(s, infer, argv):
        ...
        CHead._lookup(infer, at, **kw)

    @staticmethod
    def _lookup(infer, at, ident = '', dump = None):
        for impl, (rel, aligned, offset, size, gran) in HeMan().lookup(at):
            ...
            if dump is not None and size:
                raw = infer.readvar(aligned, size, gdbval = False)
